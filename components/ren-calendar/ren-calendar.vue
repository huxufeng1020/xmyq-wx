<template>
    <view class="calendar-wrapper" v-if="loadingBoo">
        ...
        <view :class="{ hide: !monthOpen }" class="content" :style="{ height: height }">
            <view :style="{ top: positionTop + 'rpx' }" class="days">
                <view class="item" v-for="(item, index) in dates" :key="index">
                    <view
                        class="day"
                        @click="selectOne(item,isWorkDay(item.year, item.month, item.date), $event)"
                        :class="{
        ...
        <view class="padding-lr padding-top padding-bottom-xs" @click="toggle">
					<image src="https://i.loli.net/2020/07/16/2MmZsucVTlRjSwK.png" mode="scaleToFill" v-if="collapsible" class="weektoggle" :class="{ down: monthOpen }"></image>
				</view>
    </view>
</template>
    ...
    methods: {
        ...
        isToday(y, m, d) {
            ...
            //let today = this.getToday().date;  //当前日期
						let today=this.WorkArray[0]      //有数据列表第一个数据
            if (checkD == today) {
                return true;
            } else {
                return false;
        ...
        changeMonth(type){
            ...
            this.dates = this.monthDay(this.y, this.m);
						let dateYm=this.y+'-'+this.formatNum(this.m)
