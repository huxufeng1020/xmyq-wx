...
<script>
	...
	export default {
		...
		methods: {
			init(val){
				console.log('地址的value值',val)
				let value=val.split(",")
				this.value=value.map(item=>{
					return Number(item)
				})
				console.log('地址的value值',this.value)
			},
			confirm () {
				console.log(this.provinceIndex, this.cityIndex, this.areaIndex)
				let value=this.provinceIndex+','+this.cityIndex+','+this.areaIndex
				this.$emit('confirm', {
