...
export default {
	...
	async login(payload) {
		let token = "";
		// #ifdef MP-WEIXIN
		token = this.wxMiniProgramOauth("login", payload);
		return token;
		// #endif
		// #ifdef H5
	...
	wxOfficialAccountOauth(event = "login") {
		...
		let payloadObject = {
			host: baseUrl,
			event,
			token: (event !== "login" && user) ? uni.getStorageSync("token") : ""
		};
		let payload = encodeURIComponent(JSON.stringify(payloadObject));
		let appid=uni.getStorageSync('appid')
		let redirect_uri = encodeURIComponent(`${baseApiUrl}third/wxOfficialAccountOauth?payload=${payload}`);
	...
	wxOfficialAccountBaseLogin() {
		...
		window.location = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + appid +
			`&redirect_uri=${baseApiUrl}third/wxOfficialAccountBaseLogin&response_type=code&scope=snsapi_base&state=${state}`;
		throw "stop";
	},
	// #endif
	...
	async getWxMiniProgramSessionKey(autoLogin = true) { 
		...
		const res = await api("third.getWxMiniProgramSessionKey", {
	...
	async wxMiniProgramOauth(event = "login", payload) {
		...
		if (typeof session_id !== 'undefined') {
			//没有存session_id 重新获取一下。
			await this.getWxMiniProgramSessionKey(false);
		}
	
		const res = await api("third.wxMiniProgramOauth", {
			event,
