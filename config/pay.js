...
export default class ShoproPay {
	...
	constructor(payment, order, orderType,stageType) {
	...
	prepay() {
		...
		return new Promise((resolve, reject) => {
			let return_url = `${baseUrl}pages/order/payment/result?orderId=***&type=${that.payment}&payState=success&orderType=${that.orderType}`
			let params = {
	...
	async wechatWapPay() {
		...
		if (result.code === 1) {
			...
			let domain = baseUrl//store.getters.initShop.domain; //域名需要https
			let params = encodeURIComponent(
				`${domain}pages/advance/result?orderId=${that.order.id}&type=${that.payment}&orderType=${that.orderType}&payState=paying`
			)
	...
	payResult(resultType) {
		...
		uni.navigateTo({
			url:`/pages/advance/result?orderId=${that.order.id}&type=${that.payment}&orderType=${that.orderType}&payState=${resultType}`
		})
		// const that = this;
		// $Router.replace({
